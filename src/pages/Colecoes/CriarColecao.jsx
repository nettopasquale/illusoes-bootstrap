import { useEffect, useState, useContext } from "react";
import {
  Container,
  Row,
  Col,
  Form,
  Button,
  Card,
  Alert,
} from "react-bootstrap";
import {
  PlusCircle,
  ArrowLeft,
  Collection,
  PencilSquare,
} from "react-bootstrap-icons";
import { Navegacao } from "../../components/Navegacao/Navegacao";
import LayoutGeral from "../../components/LayoutGeral/LayoutGeral";
import { cloudinaryUpload } from "../../utils/cloudinaryUpload";
import { useParams } from "react-router-dom";
import { AuthContext } from "../../context/AuthContext";
import { ModalEditarColecao } from "../../components/ModalEditarColecao/ModalEditarColecao";
import { useColecao } from "../../hooks/useColecao";
import api from "../../services/api";
import "./Colecao.css"

export default function CriarColecao() {
  const { colecaoId } = useParams();
  // const [uploadingCapa, setUploadingCapa] = useState(false);
  // const [showModal, setShowModal] = useState(false);
  // const [showConfirmDelete, setShowConfirmDelete] = useState(false);
  const { usuario } = useContext(AuthContext);
  
  const modoEdicao = !!colecaoId;
  const [loading, setLoading] = useState(modoEdicao);

  const {
    nome,
    setNome,
    descricao,
    setDescricao,
    capa,
    setCapa,
    mensagem, 
    erro, 
    navigate, 
    handleCapa, 
    publicarColecao
  } = useColecao(colecaoId, modoEdicao);

  // return (
  //   <LayoutGeral>
  //     <section id="artigo" className="block artigo-block">
  //       <Container className="my-5">
  //         <Row className="mb-4 align-items-center">
  //           <Navegacao
  //             itens={[
  //               { label: "Home", to: "/" },
  //               { label: "Meu Perfil", to: "/dashboard" },
  //               { label: "Colecoes", to: "/colecoes" },
  //               { label: "Criar Colecao" },
  //             ]}
  //           />
  //           <h2 className="mb-4 fs-1 fw-bold">
  //             {modoEdicao ? "Editar Coleção" : "Criar Coleção"}
  //           </h2>
  //           {mensagem && <Alert variant="success">{mensagem}</Alert>}
  //           {erro && <Alert variant="danger">{erro}</Alert>}
  //           <Col>
  //             <h3 className="fw-bold text-primary d-flex align-items-center">
  //               {modoEdicao ? (
  //                 <>
  //                   <Collection className="me-2" />
  //                   Editar Coleção
  //                 </>
  //               ) : (
  //                 <>
  //                   <Collection className="me-2" />
  //                   Criar Nova Coleção
  //                 </>
  //               )}
  //             </h3>
  //           </Col>
  //           <Col className="text-end">
  //             <Button
  //               className="fw-bold fs-4 bg-black w-25"
  //               onClick={() => navigate("/colecoes")}
  //             >
  //               <ArrowLeft className="me-1" /> Voltar
  //             </Button>
  //           </Col>
  //         </Row>

  //         <Card className="shadow-sm border-0 rounded-3 p-4">
  //           <Form onSubmit={publicarColecao}>
  //             <Form.Group className="mb-3">
  //               <Form.Label>Nome da Coleção</Form.Label>
  //               <Form.Control
  //                 type="text"
  //                 name="nome"
  //                 value={nome}
  //                 onChange={(e) => setNome(e.target.value)}
  //                 placeholder="Ex: Coleção Raras 2025"
  //                 required
  //               />
  //             </Form.Group>

  //             <Form.Group className="mb-3">
  //               <Form.Label>Descrição</Form.Label>
  //               <Form.Control
  //                 as="textarea"
  //                 name="descricao"
  //                 rows={3}
  //                 value={descricao}
  //                 onChange={(e) => setDescricao(e.target.value)}
  //                 placeholder="Descreva brevemente sua coleção..."
  //               />
  //             </Form.Group>

  //             <Form.Group className="mb-3">
  //               <Form.Label>Capa da coleção</Form.Label>
  //               <Form.Control
  //                 name="capa"
  //                 type="file"
  //                 accept="image/"
  //                 className="w-100"
  //                 rows={3}
  //                 onChange={handleCapa}
  //                 placeholder="Coloque uma capa para sua coleção"
  //               />
  //             </Form.Group>

  //             {/* prévia da capa */}
  //             {capa && (
  //               <img
  //                 src={capa}
  //                 alt="preview"
  //                 style={{ width: "200px", marginTop: "10px" }}
  //               />
  //             )}

  //             <div className="d-flex justify-content-between mt-4 gap-5">
  //               {modoEdicao && (
  //                 <Button
  //                   className="p-5 fw-bold fs-3 bg-black w-50"
  //                   onClick={() =>
  //                     navigate(`/colecoes/${colecaoId}/cartas/editar`)
  //                   }
  //                 >
  //                   <PlusCircle className="me-1" />
  //                   Editar Cartas
  //                 </Button>
  //               )}
  //               {modoEdicao ? (
  //                 <Button
  //                   className="p-5 fw-bold fs-3 bg-black w-50"
  //                   type="submit"
  //                 >
  //                   <PlusCircle className="me-1" />
  //                   Salvar Coleção
  //                 </Button>
  //               ) : (
  //                 <Button
  //                   className="p-5 fw-bold fs-3 bg-black w-50"
  //                   type="submit"
  //                 >
  //                   <PlusCircle className="me-1" />
  //                   Criar Coleção
  //                 </Button>
  //               )}
  //             </div>
  //           </Form>
  //         </Card>
  //       </Container>
  //     </section>
  //   </LayoutGeral>
  // );

  return (
    <LayoutGeral>
      <section className="colecao-section">
        <Container style={{ maxWidth: 680 }}>
          <Navegacao
            itens={[
              { label: "Home", to: "/" },
              { label: "Coleções", to: "/colecoes" },
              { label: modoEdicao ? "Editar coleção" : "Nova coleção" },
            ]}
          />

          {/* Header */}
          <div className="colecao-page-header">
            <h1 className="colecao-page-title">
              {modoEdicao ? (
                <>
                  <PencilSquare size={20} /> Editar Coleção
                </>
              ) : (
                <>
                  <Collection size={20} /> Nova Coleção
                </>
              )}
            </h1>
            <Button
              variant="outline-secondary"
              size="sm"
              onClick={() => navigate("/colecoes")}
            >
              <ArrowLeft className="me-1" size={13} />
              Voltar
            </Button>
          </div>

          {mensagem && (
            <Alert variant="success" className="mb-3">
              {mensagem}
            </Alert>
          )}
          {erro && (
            <Alert variant="danger" className="mb-3">
              {erro}
            </Alert>
          )}

          {/* Formulário */}
          <div className="form-card">
            <Form onSubmit={publicarColecao}>
              <Form.Group className="mb-3">
                <Form.Label>Nome da coleção</Form.Label>
                <Form.Control
                  type="text"
                  value={nome}
                  onChange={(e) => setNome(e.target.value)}
                  placeholder="Ex: Raras 2025"
                  required
                />
              </Form.Group>

              <Form.Group className="mb-3">
                <Form.Label>Descrição</Form.Label>
                <Form.Control
                  as="textarea"
                  rows={3}
                  value={descricao}
                  onChange={(e) => setDescricao(e.target.value)}
                  placeholder="Descreva brevemente sua coleção..."
                  style={{ resize: "vertical" }}
                />
              </Form.Group>

              <Form.Group className="mb-4">
                <Form.Label>Capa da coleção</Form.Label>
                <Form.Control
                  type="file"
                  accept="image/*"
                  onChange={handleCapa}
                />
                {capa && (
                  <img src={capa} alt="preview" className="capa-preview" />
                )}
              </Form.Group>

              <div className="d-flex gap-2 justify-content-end">
                {/* Só no modo edição — atalho para editar cartas */}
                {modoEdicao && (
                  <Button
                    variant="outline-secondary"
                    size="sm"
                    onClick={() =>
                      navigate(`/colecoes/${colecaoId}/cartas/editar`)
                    }
                  >
                    <PlusCircle className="me-1" size={13} />
                    Editar cartas
                  </Button>
                )}

                <Button
                  type="submit"
                  variant="primary"
                  size="sm"
                  className="px-4"
                >
                  {modoEdicao ? "Salvar alterações" : "Criar coleção"}
                </Button>
              </div>
            </Form>
          </div>
        </Container>
      </section>
    </LayoutGeral>
  );
}
